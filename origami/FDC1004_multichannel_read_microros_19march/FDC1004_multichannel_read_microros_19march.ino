...
//home wifi
//#define WIFI_SSID "ARRIS-D721"
//#define WIFI_PASS "647184476623"

//Edema_Optitrack
#define WIFI_SSID "TP-Link_C7A0"
#define WIFI_PASS "87304848"
#define ROS_IP "192.168.0.101"  // Change to match your ROS2 computer's IP
#define ROS_PORT 8888          // Same as in the agent command
...
void setup() {
  set_microros_wifi_transports(WIFI_SSID, WIFI_PASS, ROS_IP, ROS_PORT);  //Initialize micro-ros transport
  ...
  rclc_publisher_init_best_effort(
    ...
    "micro_ros_arduino_node_wifipublisher");
...
void loop() {  
 ...
 // Update the measTime with the current time in milliseconds
  ...
  char strBuffer[256];  // Make sure the buffer is large enough
  snprintf(strBuffer, sizeof(strBuffer), "Cap A, Time: %lu, Capacitance: %.4f", measTime, capA);
  
  // Assign the string to the message
  msgStringA.data.data = strBuffer;
  msgStringA.data.size = strlen(strBuffer);
  ...
  // Publish the message
  rcl_publish(&string_publisher, &msgStringA, NULL); // Publish capA and capB message
