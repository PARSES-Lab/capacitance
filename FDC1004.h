...
class FDC1004 {
 public:
    ...
    void I2CSetup();
    void absoluteCapacitance(uint8_t currChan, bool measType, uint8_t * capdacPtr, unsigned long * measTime, float * capacitanceAbsolute, uint8_t readRate);
    uint8_t setCAPDAC(uint8_t currChan, uint8_t measType, uint8_t * capdacPtr, uint8_t readRate);
    uint8_t _addr;
 ...
 private:
    ...
    float configTrigRead(uint8_t measSlot, uint8_t chanA, uint8_t chanB, uint8_t measType, uint8_t capdac, uint8_t readRate);
};
